 ...
 */
class BlogSubcategoryRequest extends FormRequest
{
    /**
    ...
    {
        $subcategoryId = $this->route('blog_subcategory') ? $this->route('blog_subcategory')->id : null;
        ...
        return [
            ...
            // Slug - unique within the same category
            'slug' => [
                'required',
                'string',
                'max:255',
                'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/',
                Rule::unique('blog_subcategories', 'slug')
                    ->where('blog_category_id', $this->input('blog_category_id'))
    ...
    {
        return [
            'blog_category_id.required' => 'A categoria é obrigatória.',
            'blog_category_id.integer' => 'A categoria deve ser um número válido.',
            'blog_category_id.exists' => 'A categoria selecionada não existe.',
            ...
            'slug.unique' => 'Este slug já está sendo usado por outra subcategoria nesta categoria.',
    ...
    protected function prepareForValidation(): void
    {
        ...
        // Set category from query parameter if provided (for create form)
        if (empty($this->input('blog_category_id')) && $this->query('category')) {
            $this->merge([
                'blog_category_id' => $this->query('category'),
            ]);
        }
    }
    ...
    public function validated($key = null, $default = null): array
    {
        ...
        $validated['blog_category_id'] = (int) $validated['blog_category_id'];
